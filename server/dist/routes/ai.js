...
Keep it concise and actionable.`;
        ...
        const parsedJson = typeof content === 'string' ? safeJson(content) : content;
        if (!parsedJson)
            return res.status(500).json({ ok: false, error: 'Briefing returned invalid JSON' });
...
aiRouter.post('/debrief', async (req, res) => {
    try {
        ...
        const opportunity = opportunityId ? state.opportunities.find((o) => o.id === opportunityId) : null;
        ...
        const movements = state.movements
            ...
            .sort((a, b) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime())
            .slice(0, 8);
        const contacts = state.contacts.filter((c) => c.accountId === accountId).slice(0, 20);
        const opportunities = state.opportunities.filter((o) => o.accountId === accountId).slice(0, 20);
...
- Keep output short and usable (bullets).`;
        ...
        const parsedJson = typeof content === 'string' ? safeJson(content) : content;
        if (!parsedJson)
            return res.status(500).json({ ok: false, error: 'Debrief returned invalid JSON' });
        return res.json(parsedJson);
    }
    catch (error) {
...
- Keep it concise and usable.`;
        ...
        const parsedJson = typeof content === 'string' ? safeJson(content) : content;
        if (!parsedJson)
            return res.status(500).json({ ok: false, error: 'Gameplan returned invalid JSON' });
...
async function fetchExternalIntel(company) {
    if (!company)
        return { news: [], companyInfo: {}, signals: [] };
    const news = await fetchGdeltNews(company).catch(() => []);
    const signals = [];
    if (news.length)
        signals.push(`Recent news mentions: ${news.length}`);
    if (/city of|county|usd\s?\d|public works/i.test(company))
        signals.push('Public-sector entity likely — check procurement/capital plans');
    return {
        news,
        companyInfo: {},
        signals
...
async function fetchGdeltNews(query) {
    ...
    const response = await fetch(url, { headers: { 'User-Agent': 'kse-tools-server' } });
    if (!response.ok)
        return [];
    const data = await response.json().catch(() => null);
