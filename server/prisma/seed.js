    ...
    if (fs.existsSync(accountPath)) {
        ...
        for (const acc of accounts) {
            ...
            await prisma.account.upsert({
                ...
                update: {},
                create: {
                    id, name: acc.name, industry: acc.industry||null, city: acc.city||null,
                    state: acc.state||null, annualPotential: acc.annualPotential||null,
                    projectedValue: acc.projectedValue||null, nextStep: acc.nextStep||null,
                    notes: acc.notes||null, ownerName: acc.accountOwner||null,
                    relationshipHealth: acc.priority||null, stalled: false, stage: 'Prospect'
                }
            });
        }
    ...
    if (fs.existsSync(oppPath)) {
        ...
        for (const opp of opps) {
            await prisma.opportunity.upsert({
                ...
                update: {},
                create: {
                    id: opp.id, projectName: opp.title, description: opp.summary, value: opp.value,
                    status: 'Active', stage: 'Identified',
